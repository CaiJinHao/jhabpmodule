...
namespace Jh.Abp.QuickComponents
{
    ...
    public class JhAuthorizationMiddlewareResultHandler : IAuthorizationMiddlewareResultHandler, ITransientDependency
    {
        ...
        protected AbpExceptionHandlingOptions AbpExceptionHandlingOptions { get; }
        public JhAuthorizationMiddlewareResultHandler(IOptions<AbpExceptionHandlingOptions> options)
        {
            AbpExceptionHandlingOptions = options.Value;
        }
        ...
        private async Task HandleAndWrapException(HttpContext httpContext, Exception exception)
        {
            ...
            var data = jsonSerializer.Serialize(
                    new RemoteServiceErrorResponse(
                        ...
                        {
                            options.SendExceptionsDetailsToClients = AbpExceptionHandlingOptions.SendExceptionsDetailsToClients;
                            options.SendStackTraceToClients = AbpExceptionHandlingOptions.SendStackTraceToClients;
                        })
                    )
                );
