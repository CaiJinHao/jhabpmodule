    ...
    var datatable = {
        ...
        renderTable: function (_data, _param) {
            ...
            ajaxmod.requestAuthorize({
                ...
                apiOption:paramDefault.apiOption,
        ...
        edit: function (_options) {
            ...
            optDefault.area = _the.getWidthHeight(optDefault.area);
        ...
        getWidthHeight: function (_arr, win) {
            if (!_arr)
                _arr = ['850px', '500px'];
            var oldarr = [_arr[0],_arr[1]];
            if (!win)
                win = top.window;
            var maxWidth = ($(win).width())/1.1;
            var maxHeight = $(win).height()/1.3;
            if (parseInt(_arr[0].replace(/px/, "")) > maxWidth)
                _arr[0] = maxWidth.toString() + "px";
            if (parseInt(_arr[1].replace(/px/, "")) > maxHeight)
                _arr[1] = maxHeight.toString() + "px";
            console.log('弹窗申请大小', oldarr, '最大可用宽高', [maxWidth, maxHeight],'返回大小', _arr);
            return _arr;
        }
    };
